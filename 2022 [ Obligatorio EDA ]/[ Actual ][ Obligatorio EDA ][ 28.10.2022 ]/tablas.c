...
void insertarTabla(Tablas & ts, char *nombreTabla, Tabla t){
// Inserta la tabla t en ts en orden alfabetico segun nombreTabla
    if (ts == NULL) {//si no hay ninguna tabla la agrego al comienzo
        ...
        insertarTabla(ts->izq, nombreTabla, t);
    else if (strcmp(ts->nombre, nombreTabla) < 0) //si ts->nombre es menor 
        insertarTabla(ts->der,nombreTabla, t);
}
...
// devuelve la tabla con nombre nombreTabla
	if (ts == NULL) {
		...
		return tablaSleccionada(ts->izq, nombreTabla);
	else
		return tablaSleccionada(ts->der, nombreTabla);
}
...
/*TipoRet eliminarTupla(Tabla &t, char* cond){
// elimina las tuplas
	Tabla aux = ts;
	if( aux != NULL ){
		if (strcmp(cond, "") == 0) {
			...
			condicion c = crearCondicion(cond, ts->t);
			if (c == NULL)
				return ERROR;
			delete(aux);
			return OK;
...
// destruye el conjunto de tablas de la base de datos
	if (ts != NULL){
		destruirTablas(ts->izq);
		destruirTablas(ts->der);
		destruirTabla(ts->t);
		delete(ts);
	}
