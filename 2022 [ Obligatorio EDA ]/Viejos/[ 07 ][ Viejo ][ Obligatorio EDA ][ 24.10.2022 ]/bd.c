...
TipoRet createTable(bd &bd, char *nombreTabla){
	...
	if (!perteneceTabla(bd->ts, nombreTabla)){// si no hay una tabla con el mismo nombre, inserta
		...
		insertarTabla(bd->ts, nombreTabla, t); // inserta en las tablas base de datos una tabla vacia con nombreTabla
		return OK;
	}
	else {
		cout << "No se pudo crear la tabla ya que ya existe una con el mismo nombre /n";
		return ERROR;
	}
}
	...
	if(agregaCol != NULL) { // si existe la tabla
		...
		if(!esVaciaListaFila(primerFiladeFilas(agregaCol)) && clas != ANY){// si ya hay filas en la tabla TIENE que ser ANY
			...
			if (clas == PRIMARY_KEY && existePKenTabla(agregaCol)){ // si lo que me dan es PK y ya existe uno
				...
				TipoDato tipo = returnTipoDato_(tipoCol);
				ListaColumnas c = primeraColumna(agregaCol);
				int x = posicionColumna(c, NombreCol); // si devuelve 0 es porque la columna no existe
...
TipoRet insertInto(bd &bd, char *nombreTabla, char *columnasTupla, char *valoresTupla){
	...
	if(aInsertar != NULL){ // si existe la tabla
		...
		char *pch = strtok(columnasTupla, ":");
		while (pch != NULL) {
			if(perteneceColumna(lc, pch)){
				char * pch2 = strtok(valoresTupla, ":");
				if(pch2 != NULL){
					// aca hay que usar esta : insertarTupla(Tabla &t, int pos, char * valorDato)
					// OJO ACA HAY UN PROBLEMA, VA A CREAR UNA LISTA EMPTY CADA VEZ Y NO ES LO QUE QUEREMOS!!!!!
