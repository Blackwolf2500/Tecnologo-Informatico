...
void insertarTabla (Tablas & ts, char *nombreTabla, Tabla tab){
// Inserta la tabla t en ts en orden alfabetico segun nombreTabla
    if (ts == NULL) {//si no hay ninguna tabla la agrego al comienzo
        ...
        insertarTabla(ts->izq, nombreTabla, tab);
    else if (strcmp(ts->nombre, nombreTabla) < 0) //si ts->nombre es menor 
        insertarTabla(ts->der,nombreTabla, tab);
    else // si es igual 
...
void eliminarTupla(Tabla &t, char* cond){
// elimina las tuplas
